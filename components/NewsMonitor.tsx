...
export default function NewsMonitor() {
    ...
    const [reportType, setReportType] = useState<"company_news" | "sector_weekly" | "">("");
    ...
    return <main className="news-monitor">
        ...
        <div className="news-filters">
            ...
            <label>種別<select value={reportType} onChange={(e) => setReportType(e.target.value as typeof reportType)}><option value="">すべて</option><option value="company_news">会社ニュース</option><option value="sector_weekly">業種レポート</option></select></label>
            ...
            <label>会社カテゴリ<input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="orders" disabled={reportType === "sector_weekly"} /></label>
            <label>決算関連<select value={relevance} onChange={(e) => setRelevance(e.target.value as EarningsRelevance | "")} disabled={reportType === "sector_weekly"}><option value="">すべて</option>{["direct","likely","general","context","unknown"].map((v) => <option key={v}>{v}</option>)}</select></label>
        ...
        <div className="news-monitor-layout"><section className="news-feed" aria-live="polite">
            ...
            {rows.map((row) => {
                ...
                return <article className={`news-card ${sector ? "sector-report-card" : `temporal-${row.temporal_status}`}`} key={row.stream_id} onClick={() => setSelected(row)} onKeyDown={(e) => { if (e.key === "Enter") setSelected(row); }} role="button" tabIndex={0}>
                    <div className="news-card-meta"><time>{dateTime(row.sort_at)}</time>{sector ? <span>東証33業種</span> : <Link href={`/?ticker=${encodeURIComponent(row.ticker)}`} onClick={(e) => e.stopPropagation()}>{row.ticker} {names.get(row.ticker) ?? row.company_name ?? ""}</Link>}{lastSeen && row.created_at > lastSeen && <b className="new-badge">NEW</b>}</div>
                    <h2>{row.title}</h2><div className="news-badges"><span>{row.category}</span><span className={`news-badge direction-${row.direction}`}>{row.direction}</span><span className={`news-badge importance-${row.importance.replace("+", "-plus")}`}>{row.importance}</span>{!sector && <span>{row.earnings_relevance}</span>}</div>
                    {sector ? <ul className="sector-summary-bullets">{row.summary_bullets.map((bullet, index) => <li key={index}>{bullet}</li>)}</ul> : <p>{row.summary}</p>}
                    <small>{sector ? "東証33業種週次レポート" : row.source_name}</small>
                </article>;
            })}
        ...
        <aside className="news-detail">{selected ? <><button className="news-detail-close" onClick={() => setSelected(null)}>×</button>{isSectorReport(selected) ? <SectorDetail row={selected} /> : <CompanyDetail row={selected} names={names} />}</> : <p className="news-empty">ニュースを選択すると詳細を表示します</p>}</aside></div>
    </main>;
}

function CompanyDetail({ row, names }: { row: Exclude<NewsStreamItem, { report_type: "sector_weekly" }>; names: Map<string, string> }) {
