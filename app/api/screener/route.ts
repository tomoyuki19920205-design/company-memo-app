...
export async function GET(request: NextRequest) {
    ...
    if (params.get("mode") === "options") {
        const { data, error } = await supabase.from("screener_metrics_current").select("market_code,market_name,sector17_code,sector17_name,sector33_code,sector33_name");
        if (error) return NextResponse.json({ error: error.message }, { status: 500 });
        const optionRows = (data ?? []) as Array<Record<string, unknown>>;
