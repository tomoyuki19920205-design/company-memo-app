...
| 単位 | DB保存値は**百万円統一**（変換後）、元単位は `unit` カラムに記録 |
...
CREATE TABLE IF NOT EXISTS edinet_order_data (
    ...
    -- 受注系指標（百万円統一）
    orders_received         numeric     NULL,       -- 受注高
    order_backlog           numeric     NULL,       -- 受注残高（期末）
    construction_carryover  numeric     NULL,       -- 繰越工事高（建設業系）
    completed_construction  numeric     NULL,       -- 完成工事高（建設業系）
    rpo                     numeric     NULL,       -- RPO（Remaining Performance Obligation）
    ...
    unit                    text        NOT NULL DEFAULT 'million_yen',
                                                    -- 変換前の元の単位を記録。
                                                    -- DB格納値は常に百万円。
                                                    -- 値: 'million_yen' | 'yen' | 'billion_yen' | 'thousand_yen' | 'unknown'
...
| 元単位 | 変換処理 | unit カラム値 |
|---|---|---|
| 百万円 | そのまま | `million_yen` |
| 億円 | × 100 | `billion_yen` |
| 千円 | ÷ 1,000 | `thousand_yen` |
| 円 | ÷ 1,000,000 | `yen` |
| 不明 | NULL で保存 | `unknown` |
...
| `ticker` | text | ✅ | — | 銘柄コード（4桁） |
| `company_name` | text | — | NULL | 表示用社名（companies JOIN で代替可） |
| `doc_id` | text | — | NULL | EDINET docID |
| `period` | text | ✅ | — | 決算年度文字列（例: '2024'） |
| `fiscal_year` | integer | ✅ | — | 決算年度整数（例: 2024） |
| `orders_received` | numeric | — | NULL | 受注高（百万円） |
| `order_backlog` | numeric | — | NULL | 受注残高（百万円） |
| `construction_carryover` | numeric | — | NULL | 繰越工事高（百万円） |
| `completed_construction` | numeric | — | NULL | 完成工事高（百万円） |
| `rpo` | numeric | — | NULL | RPO（百万円） |
| `unit` | text | ✅ | `'million_yen'` | 変換前の元の単位 |
| `segment_name` | text | — | NULL | NULL=連結全体、非NULL=セグメント名 |
| `source_type` | text | ✅ | `'edinet_yuho'` | データソース種別 |
| `source_tag` | text | — | NULL | 有報内の参照箇所タグ |
| `confidence` | text | ✅ | `'high'` | 抽出信頼度（high/medium/low） |
| `null_reason` | text | — | NULL | NULL値・low confidenceの理由 |
| `snippet` | text | — | NULL | 抽出元の原文スニペット（検証用） |
...
**ファイル名**: `migrations/009_edinet_order_data.sql`  
**※ 現時点では作成していない。この設計書が承認されてから作成する。**

```sql
-- ============================================================
-- 009_edinet_order_data.sql — EDINET受注系データテーブル
-- EDINET有報から抽出した受注高・受注残高・RPO等を格納
-- ※ 設計段階のみ。本番適用前にレビュー必須。
-- ============================================================
...
-- UNIQUE 制約（PostgreSQL 15+ 前提: NULLS NOT DISTINCT）
ALTER TABLE edinet_order_data
    ADD CONSTRAINT edinet_order_data_uniq
    UNIQUE NULLS NOT DISTINCT (ticker, period, fiscal_year, segment_name, source_type);
...
-- RLS（SELECT: allowed_users のみ / INSERT・UPDATE: service_role_key 経由のみ）
